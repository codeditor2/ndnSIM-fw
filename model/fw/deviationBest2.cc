...
//NS_LOG_COMPONENT_DEFINE ("ndn.fw.GreenYellowRed.DeviationBest2");

namespace ns3 {
namespace ndn {
namespace fw {

NS_OBJECT_ENSURE_REGISTERED (DeviationBest2);

LogComponent DeviationBest2::g_log = LogComponent (DeviationBest2::GetLogName ().c_str ());
...
//so the log name for this model is "ndn.fw.DeviationBest2".
//When we want to see the log of this model, use NS_LOG=ndn.fw.DeviationBest2 please.
std::string
DeviationBest2::GetLogName ()
{
  return super::GetLogName ()+".DeviationBest2";
}


TypeId
DeviationBest2::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::ndn::fw::DeviationBest2")
    .SetGroupName ("Ndn")
    .SetParent <super> ()
    .AddConstructor <DeviationBest2> ()
    ;
  return tid;
}

DeviationBest2::DeviationBest2 ()
{
//  NS_LOG_INFO ("intiate DeviationBest2");
}

bool
DeviationBest2::DoPropagateInterest (Ptr<Face> inFace,
...
// (3)
	...
	srtt_gap + (1.0-efficacy_coefficient);
      ...
      std::cout << "status: " << metricFacewithEntroy->GetStatus () << ", rtt: " << metricFacewithEntroy->GetSRtt () << "\n";
...
// 	m_pi_weight = 0.0;
    ...
    std::cout << "v1=" << status_weight << ", v2=" << m_sRtt_weight << "\n";
...
//         std::cout << metricFacewithEntroy.GetFace()->GetId() << "\n";
  ...
  else
  {
    ...
    if(interest->GetName ().get (-1).toSeqNum () > 100) {
      double max_probability=  0.0;
      double temp = 0.0;
      int loop_count = 0;
      int propagatedCount2 = 0;
      Ptr<Face> select_face;
      
      BOOST_FOREACH (const FaceMetricWithEntroy &metricFacewithEntroy, m_faces_withEntroy.get<i_score> ())
      {
	// find the max probability
	loop_count ++;
	temp = pow(metricFacewithEntroy.GetScore (),gamma) * pow(1.0/(metricFacewithEntroy.GetRoutingCost ()+1),delta) / total_score; //modified
	if (temp > max_probability ) {
	    max_probability = temp;
	    select_face = metricFacewithEntroy.GetFace();
	    propagatedCount2 = loop_count;
	}   
      }
      
      if (TrySendOutInterest (inFace, select_face, interest, pitEntry)) {
	  //
	  propagatedCount = propagatedCount2;
      }
    } else {
